from pydfs_lineup_optimizer import get_optimizer, Site, Sport
import pandas as pd
from bs4 import BeautifulSoup
import requests
import scrapeInactives
import os

# Get current working directory
cwd = os.getcwd()
salaryPath = cwd + "\DKSalaries.csv"

###Removed to allow for speed in testing
##listofinactive = scrapeInactives.scrape()

# Set up the optimizer
optimizer = get_optimizer(Site.DRAFTKINGS, Sport.BASKETBALL)
optimizer.load_players_from_csv(salaryPath)

##inactives = []
##inactives.append("Lebron James")
##inactives.append("Rajon Rondo")

##alsoRemove = ["Lebron James",
##              "Rajon Rondo"]
##
##for players in alsoRemove:
##    inactives.append(players)



##injuries = []
##for player in inactives:
##    injuries.append(optimizer.get_player_by_name(player))

##for player in injuries:
##    optimizer.remove_player(player)

##bron = optimizer.get_player_by_name("Lebron James")
player = optimizer.get_player_by_name('Lebron James')
optimizer.remove_player(player)
    
    
for lineup in optimizer.optimize(n=1, max_exposure=0.3):
    print(lineup)
    
